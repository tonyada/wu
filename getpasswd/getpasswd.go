...
func Scan() string {
	pass, err := getPasswd("", false, os.Stdin, os.Stdout)
	if err != nil {
		println("getpasswd scan err: ", err)
	}
	return string(pass)
}

func ScanMasked() string {
	pass, err := getPasswd("", true, os.Stdin, os.Stdout)
	if err != nil {
		println("getpasswd ScanMasked err: ", err)
	}
	return string(pass)
}
